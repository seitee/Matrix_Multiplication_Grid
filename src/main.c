...
void print_to_file(char *name, MATRIX *result)
{
	FILE * fp;
	fp = fopen (name,"w");
	int i, j;
	...
	for( i = 0; i < result->row; i++ ) {
    for( j = 0; j < result->column; j++ ) {
      fprintf(fp, "%d ", (result->matrix)[ i ][ j ] );
    }
    fprintf(fp,"\n");
  	}
...
int main( int argc, char **argv ) {
  if( argc != 4 ) { // If arguments count not corrent show error and exit
    ...
    printf( "The proper format is: a.out filename1 filename2 outputname\n\n" );
  } else { 
    FILE *in1_file = fopen( argv[1], "r" );
    FILE *in2_file = fopen( argv[2], "r" );
